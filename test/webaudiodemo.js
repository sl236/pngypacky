...
boot = function()
{
	...
	if( !Mixer.Init() )
	{
        $('body').html("Audio init failed!");
		return;
	}
    
    $('head').append( $('<link rel="stylesheet" type="text/css" />').attr('href',PackedFiles["test/data/style.css"]) );
    
    var mod = PackedFiles["test/data/gslinger.mod"].replace(/^.+,/,'');
    gotModule(mod);
}
// -------------------
})();
